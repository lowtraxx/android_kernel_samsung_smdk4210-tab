...
static int gsc_src_set_buf_seq(struct gsc_context *ctx, u32 buf_idx,
	enum drm_exynos_ipp_buf_ctrl buf_ctrl)
{
	struct exynos_drm_ippdrv *ippdrv = &ctx->ippdrv;
	bool masked;
	u32 cfg;
	u32 mask = 0x00000001 << buf_idx;

	DRM_DEBUG_KMS("%s:buf_idx[%d]buf_ctrl[%d]\n", __func__,
		buf_idx, buf_ctrl);
			...
			enum drm_exynos_ipp_buf_ctrl buf_ctrl)
{
	...
	case IPP_BUF_CTRL_UNMAP:
		...
		gsc_write(buf_info->base[EXYNOS_DRM_PLANAR_Y],
			GSC_IN_BASE_ADDR_Y(id));
		gsc_write(buf_info->base[EXYNOS_DRM_PLANAR_CB],
			GSC_IN_BASE_ADDR_CB(id));
		gsc_write(buf_info->base[EXYNOS_DRM_PLANAR_CR],
			GSC_IN_BASE_ADDR_CR(id));
		break;
	default:
		/* bypass */
		break;
...
static int gsc_dst_set_buf_seq(struct gsc_context *ctx, u32 buf_idx,
	enum drm_exynos_ipp_buf_ctrl buf_ctrl)
{
	struct exynos_drm_ippdrv *ippdrv = &ctx->ippdrv;
	bool masked;
			...
			enum drm_exynos_ipp_buf_ctrl buf_ctrl)
{
	...
	case IPP_BUF_CTRL_UNMAP:
		...
		gsc_write(buf_info->base[EXYNOS_DRM_PLANAR_Y],
			GSC_OUT_BASE_ADDR_Y(id));
		gsc_write(buf_info->base[EXYNOS_DRM_PLANAR_CB],
			GSC_OUT_BASE_ADDR_CB(id));
		gsc_write(buf_info->base[EXYNOS_DRM_PLANAR_CR],
			GSC_OUT_BASE_ADDR_CR(id));
		break;
	default:
		/* bypass */
		break;
...
static irqreturn_t gsc_irq_handler(int irq, void *dev_id)
{
	...
	if (gsc_dst_set_buf_seq(ctx, buf_idx,
