...
static irqreturn_t ssp_irq_thread_fn(int iIrq, void *dev_id)
{
	struct ssp_data *data = dev_id;

	data_dbg("%s\n", __func__);
	ssp_select_irq_msg(data);
...
static int initialize_irq(struct ssp_data *data)
{
	...
	iRet = request_threaded_irq(iIrq, NULL, ssp_irq_thread_fn,
	...
	const struct i2c_device_id *devid)
{
	...
	ssp_set_sensor_position(data);
	ssp_set_proximity_threshold(data);
