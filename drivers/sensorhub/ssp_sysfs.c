...
static u8 get_delay_cmd(u8 uDelayRate)
{
	...
	int iSensorType, int64_t dNewDelay)
{
	...
	case ADD_SENSOR_STATE:
		ssp_dbg("%s: [SSP] add %u, New = %lldns\n",
			 __func__, 1 << iSensorType, dNewDelay);

		uBuf[1] = get_msdelay(dNewDelay);
		uBuf[0] = get_delay_cmd(uBuf[1]);
		ssp_send_inst(data, ADD_SENSOR, iSensorType, uBuf, 2);
	...
	case RUNNING_SENSOR_STATE:
		...
		ssp_dbg("%s: [SSP] Change %u, New = %lldns\n",
			__func__, 1 << iSensorType, dNewDelay);
		ssp_send_inst(data, CHANGE_DELAY, iSensorType, uBuf, 2);
		break;
	default:
	...
	unsigned int uChangedSensor, unsigned int uNewEnable)
{
	...
	ssp_dbg("%s: [SSP] remove sensor = %d, current state = %d\n",
	...
	if (!uNewEnable)
		if (data->bCheckSuspend == false) {
			...
			data->bDebugEnable = false;
		}

	uBuf[1] = get_msdelay(dSensorDelay);
	uBuf[0] = get_delay_cmd(uBuf[1]);

	ssp_send_inst(data, REMOVE_SENSOR, uChangedSensor, uBuf, 2);
	data->aiCheckStatus[uChangedSensor] = NO_SENSOR_STATE;
	return 0;
}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	ssp_dbg("%s: [SSP] new_enable = %u, old_enable = %u\n", __func__,
...
static ssize_t ssp_show_gesture(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct ssp_data *data  = dev_get_drvdata(dev);
...
static DEVICE_ATTR(gesture, S_IRUGO, ssp_show_gesture, NULL);
static DEVICE_ATTR(acc_poll_delay, S_IRUGO | S_IWUSR | S_IWGRP | S_IWOTH,
	show_acc_delay, set_acc_delay);
static DEVICE_ATTR(gyro_poll_delay, S_IRUGO | S_IWUSR | S_IWGRP | S_IWOTH,
	show_gyro_delay, set_gyro_delay);
static DEVICE_ATTR(mag_poll_delay, S_IRUGO | S_IWUSR | S_IWGRP | S_IWOTH,
	show_mag_delay, set_mag_delay);
static DEVICE_ATTR(ori_poll_delay, S_IRUGO | S_IWUSR | S_IWGRP | S_IWOTH,
	show_ori_delay, set_ori_delay);
static DEVICE_ATTR(pressure_poll_delay, S_IRUGO | S_IWUSR | S_IWGRP | S_IWOTH,
	show_pressure_delay, set_pressure_delay);
static DEVICE_ATTR(gesture_poll_delay, S_IRUGO | S_IWUSR | S_IWGRP | S_IWOTH,
	show_gesture_delay, set_gesture_delay);
static DEVICE_ATTR(light_poll_delay, S_IRUGO | S_IWUSR | S_IWGRP | S_IWOTH,
	show_light_delay, set_light_delay);
static DEVICE_ATTR(prox_poll_delay, S_IRUGO | S_IWUSR | S_IWGRP | S_IWOTH,
...
int initialize_sysfs(struct ssp_data *data)
{
	...
	if (sensors_register(sensor_device, data,
		sensor_attrs, "ssp_sensor"))
